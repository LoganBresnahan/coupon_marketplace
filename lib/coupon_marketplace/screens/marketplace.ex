defmodule CouponMarketplace.Screens.Marketplace do
  ...
  defp coupons_for_sale(current_state) do
    from(
      ...
      where: coupon.status == ^:available and coupon.user_id != ^current_state.user.id,
      preload: [:brand, :user],
  ...
  defp transaction(current_state) do
    with {:ok, coupon} <- get_coupon(current_state),
         ...
         :ok <- sufficient_funds(buyer.balance >= coupon.value) do
      ...
      |> case do
        ...
        _ ->
          IO.puts(
            "********** Sorry, and internal error has occured during the transaction process. **********"
          )
  ...
  defp update_seller!(seller, coupon, house_profit) do
    new_balance =
      Decimal.sub(coupon.value, house_profit)
  ...
  defp get_coupon(current_state) do
    ...
    from(
      coupon in Coupon,
      where:
        coupon.id == ^coupon_id and coupon.status == ^:available and
          coupon.user_id != ^current_state.user.id,
      select: coupon
    )
    |> Repo.one()
    |> case do
